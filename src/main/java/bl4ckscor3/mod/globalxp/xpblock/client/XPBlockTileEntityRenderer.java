...
public class XPBlockTileEntityRenderer extends BlockEntityRenderer<XPBlockTileEntity>
{
	private ItemStack emerald = new ItemStack(Items.EMERALD, 1);

	public XPBlockTileEntityRenderer(BlockEntityRenderDispatcher terd)
	{
		super(terd);
	}
	...
	public void render(XPBlockTileEntity te, float partialTicks, PoseStack stack, MultiBufferSource buffer, int combinedLight, int combinedOverlay)
	{
		...
		{
			HitResult rtr = renderer.cameraHitResult;
		...
		BakedModel model = Minecraft.getInstance().getItemRenderer().getModel(emerald, te.getLevel(), null);
